<?php
    ...
    // var_dump($_SESSION['login_user']['user_id']);
		...
		if (!empty($_POST)) {
				...
				$sql='SELECT COUNT(*) AS `cnt` FROM `cheers` WHERE `user_id`=? AND `dream_id`=?';
				$data=array($user_id,$read_users['now_dream_id']);
				$stmt=$dbh->prepare($sql);
				$stmt->execute($data);
				$cheer=$stmt->fetch(PDO::FETCH_ASSOC);
		}
        ...
        <div class="container-fluid">
          ...
          <div style="margin:10px">
          	<form method="POST" action="">
            	...
            	<input type="submit" value="応援！" class="btn btn-xs btn-primary">
            	<input type="submit" value="応援中！" class="btn btn-xs btn-primary">
            </form>
          </div>
